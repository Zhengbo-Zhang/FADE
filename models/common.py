...
class SA(nn.Module):
    ...
    def forward(self, x, ma):
        fsz = x.shape[-1]
        msz = ma.shape[-1]
        scale = msz/fsz
        self.maxpool = Maxpool(int(scale), int(scale), 0, 1)
        avg_out = torch.mean(x, dim=1, keepdim=True)
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        m = self.maxpool(ma)
        # x = torch.cat([m, avg_out, max_out], dim=1)
        # x = self.conv1(x)
        x = x.mul(self.sigmoid(self.conv1(torch.cat([m, avg_out, max_out], dim=1))))
        return x
...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, ma, augment=False, visualize=False, val=False):
        # YOLOv5 MultiBackend inference
        b, ch, h, w = im.shape  # batch, channel, height, width
        if self.pt:  # PyTorch
            y = self.model(im, ma, augment=augment, visualize=visualize)[0]
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and self.device.type != 'cpu':
            ...
            masz = (1,3,960,960)
            ma = torch.zeros(*masz, dtype=torch.half if self.fp16 else torch.float, device=self.device)
            for _ in range(2 if self.jit else 1):  #
                self.forward(im, ma)  # warmup
