...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, masks, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            masks = masks.to(device, non_blocking=True).float() / 255
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    masks = nn.functional.interpolate(masks, size=ns, mode='bilinear', align_corners=False)
            # Forward
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs, masks)  # forward
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _, tp, fp = val.run(data_dict,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='/mnt/data1/ghh/yolov5/models/yolov5x.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/paowu.yaml', help='dataset.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            'lr0': (1e-2, 1e-7, 1e-3),  # initial learning rate (SGD=1E-2, Adam=1E-3)
